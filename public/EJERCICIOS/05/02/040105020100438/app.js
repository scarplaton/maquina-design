  ...
  Promise.all(datos.map(x => cargaImagen(x.src))).then(function (imagenes) {
    ...
    for (var i = 0, centro; i < repgrupos; i++) {
      ...
      if (_mostrarValores === 'si' && (i + 1) === repgrupos) {
        ...
        ctx.beginPath();
        ctx.arc(centro-(anchoSeccion/2), yStart+separacionImg+separacionElem+20, 20, 1.5*Math.PI, 0, false);
        ctx.lineTo(centro-(anchoSeccion/2)+20, yStart+(altoTotal/2)-20);
        ctx.arc(centro-(anchoSeccion/2)+40, yStart+(altoTotal/2)-20, 20, Math.PI, .5*Math.PI, true)
        ctx.arc(centro-(anchoSeccion/2)+40, yStart+(altoTotal/2)+20, 20, 1.5*Math.PI, Math.PI, true)
        ctx.lineTo(centro-(anchoSeccion/2)+20, yStart+altoTotal-40);
        ctx.arc(centro-(anchoSeccion/2), yStart+altoTotal-separacionImg-separacionElem-20, 20, 0, .5*Math.PI, false);
        ...
        for (let repeticion of repeticiones) {
          ...
          switch (repeticion.tipoValorFinal) {
            case 'texto':
              ...
              ctx.fillText(repeticion.valorFinal, centro-(anchoSeccion/2)+40, yTexto);
              ctx.restore();
              break;
            case 'imagen':
