...
function dibujaHtml() {
  ...
  if (contenidoRespuestas.length > 0) {
    ...
    contenidoBody['r'].forEach(function (item, index) {
      ...
      if (item.tag != 'general') {
        respuestaHtml += `<canvas class="img-fluid mx-auto d-block" id="container-r${index}"></canvas>`
      } else {
  ...
  contenidoBody['g'].forEach((m, i) => {
    ...
    if (m.tag != 'general') {
      glosaHtml += `<canvas class="img-fluid mx-auto d-block" id="container-${'g'}${i}"></canvas>`
    } else {
