...
function repeticion_imagen(cantidad, imgsrc, alto, ancho){
  cantidad = Number(cantidad);
  
  let con = "";
  let imagen_editada = `<img src="${imgsrc.replace('https://desarrolloadaptatin.blob.core.windows.net/sistemaejercicios/ejercicios/Nivel-4/', '../../../../')}" height="${alto}" width="${ancho}"/>`;

  for(let i = 0; i < cantidad; i++){ 

      con += i+1 === cantidad ? imagen_editada  : `${imagen_editada} &nbsp`;
  }
  return con;
}
