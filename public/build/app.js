...
function cargaFuente(nombre, src) {
  return new Promise(function (resolve, reject) {
    var font = new FontFace(nombre, "url('".concat(src, "')"), {});
...
var FUNCIONES = [{
  ...
  fns: [{
    ...
    id: 'Insertar Input',
    action: insertarInput
  }, {
    id: 'Insertar Tabla',
    action: insertarTabla
  }, {
    id: 'Insertar Input Fraccion',
    action: insertarInputFraccion
  }]
}, {
  name: 'Datos',
  tag: 'datos',
  ...
  fns: [{
    ...
    id: 'Tabla Posicional',
    action: tablaPosicional
  }, {
    id: 'Valor Posicional',
    action: valorPosicional
  }, {
    id: 'Repetición Pictóricos',
    action: repeticionPic
  }]
}, {
  name: 'Medicion',
  tag: 'medicion',
          ...
          for (var funcionIndex = 0; funcionIndex < FUNCIONES[oaIndex].fns.length; funcionIndex++) {
            if (FUNCIONES[oaIndex].fns[funcionIndex].id === m.name) {
              ...
              FUNCIONES[oaIndex].fns[funcionIndex].action({
                container: document.getElementById("container-".concat(n).concat(i)),
                params: m.params,
...
function dibujaHtml() {
  ...
  contenidoBody['e'].forEach(function (m, i) {
    contenidoHtml += "<div class=\"col-md-".concat(m.width.md, " col-sm-").concat(m.width.sm, " col-xs-").concat(m.width.xs, " tag\">");

    if (m.tag != 'general') {
      contenidoHtml += "<canvas id=\"container-".concat('e', i, "\" class=\"img-fluid mx-auto d-block\" style=\"background:").concat(m.params.background, "\"></canvas>");
    } else {
      contenidoHtml += "<div id=\"container-".concat('e', i, "\" class=\"general\"></div>");
    }

    contenidoHtml += '</div>';
  });
  contenidoDiv.innerHTML = contenidoHtml; // INICIO RESPUESTA
  ...
  if (canvasSeleccionables.length === 3) {
    ...
    canvasSeleccionables.forEach(function (item, index) {
      ...
      var dataContent = {
        ...
        opcion: "Opcion ".concat(index + 1),
        errFrec: item.params.errFrec
      };
      respuestaHtml += "<div class=\"col-md-".concat(item.width.md, " col-sm-").concat(item.width.sm, " col-xs-").concat(item.width.xs, " radio-div text-center\">");
      respuestaHtml += "<canvas class=\"img-fluid\" id=\"container-".concat('r', item.position, "\" style=\"background:").concat(item.params.background, "\"></canvas>");
      respuestaHtml += "<h5 class=\"h5 text-center\">Opcion ".concat(index + 1, "</h5>");
      respuestaHtml += "<input name=\"answer\" value=\"Opcion ".concat(index + 1, "\" class=\"d-none\" type=\"radio\" data-content='").concat(JSON.stringify(dataContent), "' />");
      respuestaHtml += '</div>';
    });
  } else {
    contenidoBody['r'].forEach(function (item, index) {
      respuestaHtml += "<div class=\"col-md-".concat(item.width.md, " col-sm-").concat(item.width.sm, " col-xs-").concat(item.width.xs, " tag\">");

      if (item.tag != 'general') {
        respuestaHtml += "<canvas class=\"img-fluid\" id=\"container-".concat('r', index, "\" style=\"background:").concat(item.params.background, "\"></canvas>");
      } else {
        respuestaHtml += "<div id=\"container-".concat('r', index, "\" class=\"general\"></div>");
      }
  ...
  respuestaDiv.innerHTML = respuestaHtml; // INICIO GLOSA

  var glosaDiv = document.getElementById('glosa');
  var glosaHtml = '';
  contenidoBody['g'].forEach(function (m, i) {
    glosaHtml += "<div class=\"col-md-".concat(m.width.md, " col-sm-").concat(m.width.sm, " col-xs-").concat(m.width.xs, " tag\">");

    if (m.tag != 'general') {
      glosaHtml += "<canvas class=\"img-fluid mx-auto d-block\" id=\"container-".concat('g', i, "\" style=\"background:").concat(m.params.background, "\"></canvas>");
    } else {
      glosaHtml += "<div id=\"container-".concat('g', i, "\" class=\"general\"></div>");
    }
  ...
  if (container) {
    switch (inputType) {
      case 'input':
        ...
        switch (tipoInput) {
          case 'texto':
            container.innerHTML = "<input type=\"text\" name=\"answer\" maxlength=\"".concat(maxLength, "\" class=\"inputTexto\" placeholder=\"Respuesta\" data-content='").concat(JSON.stringify(dataContent), "' onkeypress=\"cambiaInputTexto(event)\" />");
            break;

          case 'numero':
            container.innerHTML = "<input type=\"text\" name=\"answer\" maxlength=\"".concat(maxLength, "\" class=\"inputTexto\" placeholder=\"Respuesta\" data-content='").concat(JSON.stringify(dataContent), "' onkeypress=\"cambiaInputNumerico(event)\" onkeyup=\"formatearNumero(event)\" />");
            break;

          case 'alfanumerico':
            container.innerHTML = "<input type=\"text\" name=\"answer\" maxlength=\"".concat(maxLength, "\" class=\"inputTexto\" placeholder=\"Respuesta\" data-content='").concat(JSON.stringify(dataContent), "' onkeypress=\"cambiaInputAlfanumerico(event)\"/>");
            break;
        }
      ...
      case 'radio':
        ...
        answers.forEach(function (m, i) {
          ...
          lmnt.className = "col-".concat(col, " col-sm-").concat(colsm, " col-md-").concat(colmd);
          lmnt.innerHTML = "<div class=\"opcionradio\">\n\t<span></span>\n\t<input type=\"radio\" id=\"radio-".concat(i, "\" name=\"answer\" value=\"").concat(m.respuesta, "\" onchange=\"cambiaRadios(event)\" data-content='").concat(JSON.stringify(m), "'>\n\t<label for=\"radio-").concat(i, "\">").concat(m.respuesta, "</label>\n</div>");
      ...
      case 'checkbox':
        arr.forEach(function (m, i) {
          ...
          try {
            ...
            r += "<li key=\"".concat(i, "\"><input name=\"answer\" value=\"").concat(n, "\" type=\"checkbox\"/><label>").concat(n, "</label></li>");
          } catch (e) {
            r += "<li key=\"".concat(i, "\"><input name=\"answer\" value=\"").concat(valoresReemplazados, "\" type=\"checkbox\"/><label>").concat(valoresReemplazados, "</label></li>");
          }
        });
        ...
        switch (table[row][col].type) {
          case 'text':
            if (encabezado === 'arriba' && row === 0) {
              r += "<p><b>".concat(regex(table[row][col].value.text, vars, vt), "</b></p>");
            } else if (encabezado === 'izquierda' && col === 0) {
              r += "<p><b>".concat(regex(table[row][col].value.text, vars, vt), "</b></p>");
            } else {
              r += "<p>".concat(regex(table[row][col].value.text, vars, vt), "</p>");
            }

            break;

          case 'image':
            r += "<img src=".concat(regex(table[row][col].value.url, vars, vt), " height=").concat(table[row][col].value.height, " width=").concat(table[row][col].value.width, "/>");
            break;
          ...
          case 'input':
            ...
            switch (tipoInput) {
              case 'text':
                r += "<input type=\"text\" name=\"answer\" maxlength=\"".concat(maxLength, "\" placeholder=\"Respuesta\" data-content='").concat(JSON.stringify(dataContent), "' onkeypress=\"cambiaInputTexto(event)\" />");
                break;

              case 'numero':
                r += "<input type=\"text\" name=\"answer\" maxlength=\"".concat(maxLength, "\" placeholder=\"Respuesta\" data-content='").concat(JSON.stringify(dataContent), "' onkeypress=\"cambiaInputNumerico(event)\" onkeyup=\"formatearNumero(event)\" />");
                break;

              case 'alfanumerico':
                r += "<input type=\"text\" name=\"answer\" maxlength=\"".concat(maxLength, "\" placeholder=\"Respuesta\" data-content='").concat(JSON.stringify(dataContent), "' onkeypress=\"cambiaInputAlfanumerico(event)\"/>");
                break;
            }
          ...
          case 'text-input':
            ...
            switch (tipoInput) {
              case 'text':
                input = "<input type=\"text\" name=\"answer\" maxlength=\"".concat(maxLength, "\" placeholder=\"Respuesta\" data-content='").concat(JSON.stringify(dataContent), "' onkeypress=\"cambiaInputTexto(event)\" />");
                break;

              case 'numero':
                input = "<input type=\"text\" name=\"answer\" maxlength=\"".concat(maxLength, "\" placeholder=\"Respuesta\" data-content='").concat(JSON.stringify(dataContent), "' onkeypress=\"cambiaInputNumerico(event)\" onkeyup=\"formatearNumero(event)\" />");
                break;

              case 'alfanumerico':
                input = "<input type=\"text\" name=\"answer\" maxlength=\"".concat(maxLength, "\" placeholder=\"Respuesta\" data-content='").concat(JSON.stringify(dataContent), "' onkeypress=\"cambiaInputAlfanumerico(event)\"/>");
                break;
            }

            r += "<p>".concat(p.replace('{input}', input), "</p>");
            break;

          case 'text-image':
            var p = regex(table[row][col].value.text, vars, vt);
            var img = "<img src=".concat(regex(table[row][col].value.url, vars, vt), " height=").concat(table[row][col].value.height, " width=").concat(table[row][col].value.width, "/>");
            r += "<p>".concat(p.replace('{imagen}', img), "</p>");
            break;
        }
...
function insertarInputFraccion(config) {
  ...
  try {
    ...
    inputFraccion += "<input type=\"text\" id=\"entero\" name=\"answer\" class=\"input-numerador\" data-content=\"{'feedbackGood':'".concat(feedbackGood, "','feedbackBad':'").concat(feedbackBad, "','esCorrecta': '").concat(entero, "'}\" ").concat(disabled, " ").concat(params.disabled === 'si' && "value=\"".concat(entero, "\""), " />");
    inputFraccion += '</td><td style="border-bottom: 2px solid black;">';
    inputFraccion += "<input type=\"text\" id=\"numerador\" name=\"answer\" class=\"input-num-y-den\" data-content=\"{'feedbackGood':'".concat(feedbackGood, "','feedbackBad':'").concat(feedbackBad, "','esCorrecta': '").concat(numerador, "'}\" ").concat(disabled, " ").concat(params.disabled === 'si' && "value=\"".concat(numerador, "\""), "\"/>");
    inputFraccion += '</td></tr><tr><td>';
    inputFraccion += "<input type=\"text\" id=\"denominador\" name=\"answer\" class=\"input-num-y-den\" data-content=\"{'feedbackGood':'".concat(feedbackGood, "','feedbackBad':'").concat(feedbackBad, "','esCorrecta': '").concat(denominador, "'}\" ").concat(disabled, " ").concat(params.disabled === 'si' && "value=\"".concat(denominador, "\""), "/>");
...
function recta(config) {
  ...
  function dibujaFraccion(posicionX) {
    ...
    ctx.font = "".concat(textHeightFraccion, "px Arial");
    ctx.fillStyle = 'black';
...
function igualPerimetro(config) {
  ...
  try {
    alto = regex("$".concat(params.alto), vars, vt);
    ancho = regex("$".concat(params.ancho), vars, vt);
...
function tablaPosicional(config) {
  ...
  try {
    if (_umil !== 'Seleccione') {
      _umil = regex("$".concat(_umil), vars, vt);
      _miles = regex("$".concat(_miles), vars, vt);
    } else {
      _umil = '0';
      _miles = '0000';
    }

    _centena = regex("$".concat(_centena), vars, vt);
    _decena = regex("$".concat(_decena), vars, vt);
    _unidad = regex("$".concat(_unidad), vars, vt);
    _centenas = regex("$".concat(_centenas), vars, vt);
    _decenas = regex("$".concat(_decenas), vars, vt);
    _numero = regex("$".concat(_numero), vars, vt);
  } catch (error) {
    console.log(error);
  }
  ...
  Promise.all([cargaImagen(imgSrcTablaCentena), cargaImagen(imgSrcTablaUMil), cargaImagen(imgSrcFlechaAbajo), cargaImagen(imgSrcSignoMas), cargaFuente('LarkeNeueThinFuente', srcFuente)]).then(function (result) {
    ...
    function dibujaNumeros(numero, centroSeccion) {
      ...
      ctx.font = "".concat(altoTexto, "pt LarkeNeueThinFuente");
    ...
    function dibujaNumerosSuma(numero, centroSeccion) {
      ctx.font = "".concat(_textoUnidades, "pt LarkeNeueThinFuente");
      var anchoTexto = ctx.measureText(numero).width;
      var xTexto = centroSeccion - anchoTexto / 2;
    ...
    function dibujaNumeroFinal() {
      ctx.font = "".concat(_textoNumeroPalabras, "pt LarkeNeueThinFuente");
...
function valorPosicional(config) {
  ...
  try {
    ...
    _centenas = regex("$".concat(_centenas), vars, vt);
    _decenas = regex("$".concat(_decenas), vars, vt);
    _unidades = regex("$".concat(_unidades), vars, vt);

    if (_tipo === 'Numero Escrito') {
      _numeroPalabras = regex("$".concat(_numeroPalabras), vars, vt);
    } else if (_tipo === 'Texto') {
  ...
  Promise.all([cargaImagen(imgSrcFlechaAbajo), cargaImagen(imgSrcSignoMas), cargaFuente('LarkeNeueThinFuente', srcFuente)]).then(function (result) {
    ...
    container.width = 850;
    ctx.font = "".concat(_altoTexo, "pt LarkeNeueThinFuente");
    ctx.textAlign = "center";
    ctx.fillStyle = '#F58220';
    ...
    if (_tipo === 'Numero Escrito') {
      ...
      for (var i = 1; i < 5; i++) {
        ...
        if (i === underline) {
          ...
          if (underline === 2) {
            ctx.fillText("".concat(_centenas, " centenas = ").concat(_centenas, "00"), xTexto, yTexto);
          } else {
            ctx.fillText("".concat(_miles, " unidades de mil = ").concat(_miles, "000"), xTexto, yTexto);
          }
        }
      }
    }
    ...
    function escribeNumeroCentro() {
      var numero = "".concat(_miles, " ").concat(_centenas).concat(_decenas).concat(_unidades);
      var x = container.width / 2;
...
function repeticionPic(config) {
  ...
  try {
    _repeticiones1 = regex("$".concat(_repeticiones1), vars, vt);
    _repeticiones2 = regex("$".concat(_repeticiones2), vars, vt);
    _repeticiones3 = regex("$".concat(_repeticiones3), vars, vt);
    _repeticiones4 = regex("$".concat(_repeticiones4), vars, vt);
    _repeticiones5 = regex("$".concat(_repeticiones5), vars, vt);
    _repeticiones6 = regex("$".concat(_repeticiones6), vars, vt);
    _repeticiones7 = regex("$".concat(_repeticiones7), vars, vt);
    _repeticiones8 = regex("$".concat(_repeticiones8), vars, vt);
  } catch (error) {
    console.log(error);
  }
  ...
  var ctx = container.getContext('2d'); //carga las imagenes y dibuja las repeticiones
  ...
  function getRepeticiones() {
    var repeticiones = [{
      imagen: _imagen1 !== '' ? buscarImagen(_imagen1) : {
        src: ''
      },
    ...
    if (_pictoricos > 1) {
      repeticiones[1] = {
        imagen: _imagen2 !== '' ? buscarImagen(_imagen2) : {
          src: ''
        },
    ...
    if (_pictoricos > 2) {
      repeticiones[2] = {
        imagen: _imagen3 !== '' ? buscarImagen(_imagen3) : {
          src: ''
        },
    ...
    if (_pictoricos > 3) {
      repeticiones[3] = {
        imagen: _imagen4 !== '' ? buscarImagen(_imagen4) : {
          src: ''
        },
    ...
    if (_pictoricos > 4) {
      repeticiones[4] = {
        imagen: _imagen5 !== '' ? buscarImagen(_imagen5) : {
          src: ''
        },
    ...
    if (_pictoricos > 5) {
      repeticiones[5] = {
        imagen: _imagen6 !== '' ? buscarImagen(_imagen6) : {
          src: ''
        },
    ...
    if (_pictoricos > 6) {
      repeticiones[6] = {
        imagen: _imagen7 !== '' ? buscarImagen(_imagen7) : {
          src: ''
        },
    ...
    if (_pictoricos > 7) {
      repeticiones[7] = {
        imagen: _imagen8 !== '' ? buscarImagen(_imagen8) : {
          src: ''
        },
