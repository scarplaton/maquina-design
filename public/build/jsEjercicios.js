...
function enviar() {
	...
	var envioIntento = JSON.stringify({
		...
		"feedback": check ? 'Respuesta Correcta' : numeroIntento < 2 ? feed.replace(/(\\n)|(\\t)/g, '').trim() : null,
		"codigoErrorComun": errFre ? errFre : 0,
		"respuesta": values,
		"glosa": numeroIntento === 2 ? $('#glosa').text().replace(/(\s\s)/g, '').replace(/(\\n)|(\\t)/g, '').trim() : null
	});
...
function cerrarFeed() {
	//siguiente ejerccio respuesta correctas
...
function cerrarFeedGlosa() {
	//siguiente ejercicio respuesta incorrecta por segunda vez
