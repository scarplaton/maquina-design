...
function muestraFeedback(esCorrecta, feedback) {
	...
	if(x.matches) { //mostrar feedback en modal
		...
		if(esCorrecta) {
			...
			var src = `https://contenedoradapt.adaptativamente.cl/fontejercicios/imagenes_front/patos/${arrCorrecta[rando]}`;
			feedbackCorrecta(src);
		} else {
			var rando = Math.floor((Math.random() *  arrIncorrecta.length));
			var src = `https://contenedoradapt.adaptativamente.cl/fontejercicios/imagenes_front/patos/${arrIncorrecta[rando]}`;
