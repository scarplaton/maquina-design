	...
	if (container) {
		switch(inputType) {
			case 'input':
				...
				switch (tipoInput) {
					case 'texto':
						container.innerHTML = `<input type="text" name="answer" maxlength="${maxLength}" autocomplete="off" class="inputTexto" placeholder="Respuesta" data-content='${JSON.stringify(dataContent)}' onkeypress="cambiaInputTexto(event)" onkeyup="checkTexts()"/>`;
						break;
					case 'numero':
						...
						break;
					case 'alfanumerico':
						container.innerHTML = `<input type="text" name="answer" maxlength="${maxLength}" autocomplete="off" class="inputTexto" placeholder="Respuesta" data-content='${JSON.stringify(dataContent)}' onkeypress="cambiaInputAlfanumerico(event)" onkeyup="checkTexts()"/>`;
						break;
				}
				break;
			case 'radio':
...
function insertarTabla(config) {
	...
	if (container) {
		let r = '<div class="table-responsive"><table class="tabla"><tbody>';
			...
			for(var col = 0; col < table[row].length; col++) {
				...
				switch(table[row][col].type) {
					...
					case 'text-input':
						...
						break;
					case 'text-image':
						var p = regexFunctions(regex(table[row][col].value.text, vars, vt));
...
function rectNumFn(config) {
  ...
  state.chart = {
    ...
    image: {
      ...
      lupa: '../../../../imagenes_front/recta_numerica/lupa_recta.svg',
      pictoImg: '../../../../imagenes_front/recta_numerica/rombo_recta.svg'
    },
    values: {
